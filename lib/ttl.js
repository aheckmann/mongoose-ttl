 ...
 *    const Cache = db.model('Cache', schema);
...
function ttl (schema, options) {
  ...
  function init (model) {
    ...
    const distinct_ = model.distinct;
    ...
    'where $where'.split(' ').forEach(function (method) {
      ...
      model[method] = function () {
        const query = fn.apply(this, arguments)
          , cond = {};
        applyTTL(cond);
  ...
  function fromNow (val) {
    const v = arguments.length ? val : ttl;
